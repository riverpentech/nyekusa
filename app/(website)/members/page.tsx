...
import { Search, GraduationCap, Building, Filter, Users, SearchX } from "lucide-react";
...
export default function Members() {
    ...
    const courses = [...new Set(displayMembers.map(m => m.course).filter(Boolean))];
    const years = [...new Set(displayMembers.map(m => m.year_of_study).filter(Boolean))];
    ...
    const hasAnyMembers = displayMembers.length > 0;
    const noMatchesFromFilters = hasAnyMembers && filtersActive && filtered.length === 0;
    const trulyNoMembers = !hasAnyMembers;
    ...
    return (
        <div>
            <PageHero
                badge="Community"
                title="Our Members"
                description="Meet the vibrant community of students that make NYEKUSA great."
            />
                            ...
                            {filtered.map((member, index) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-body font-semibold text-foreground truncate">{member.full_name || 'Unknown Member'}</h3>
