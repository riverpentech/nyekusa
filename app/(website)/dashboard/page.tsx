...
export default function DashboardPage() {
    ...
    const firstName = (me?.name ?? session?.user?.name ?? "").split(" ")[0];
    const role = (me?.role ?? (session?.user as { role?: string } | undefined)?.role) ?? "MEMBER";
        ...
        <div className="space-y-8">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-2">
                        <div className="text-2xl font-bold text-slate-900">{me?.course ?? "—"}</div>
                        ...
                        {me?.payment ? (
                            <>
                                <div className="text-2xl font-bold text-slate-900">
                                    KES {me.payment.amount.toFixed(2)}
                                </div>
                                <p className="text-xs mt-1 flex items-center gap-1">
                                    ...
                                    {me.payment.mpesaReceiptCode && ` (${me.payment.mpesaReceiptCode})`}
                                </p>
                            </>
                        ) : (
                            <>
                                <div className="text-2xl font-bold text-slate-900">—</div>
                                <p className="text-xs text-slate-500 mt-1">No payment on record</p>
                            </>
                        )}
                    </div>
                </div>

                {/* Metric 3: Account Tier */}
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div className="flex items-center justify-between space-y-0 pb-2">
                    ...
                    <div className="mt-2">
                        <div className="text-2xl font-bold text-slate-900">{role}</div>
                        ...
                        <div className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg text-sm">
                            ...
                            <div>
                                ...
                                <p className="text-slate-500 text-xs">
                                    The latest association guidelines and minutes have been updated under Resources.
                                </p>
                            </div>
                        </div>
                        <div className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg text-sm">
                            ...
                            <div>
                                ...
                                <p className="text-slate-500 text-xs">
                                    Check out the events page for location and schedule timings.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-2 leading-relaxed">
                            Ensure your social links (LinkedIn, Twitter, and GitHub) are updated on your
                            profile. This allows other members and leaders within the organization to connect
                            with you directly on projects and collaborative tasks.
                        </p>
                    </div>
                    <div className="pt-4">
                        <Link
                            href="/dashboard/profile"
                            className="inline-flex justify-center rounded-lg text-sm font-semibold py-2.5 px-4 bg-emerald-800 text-white hover:bg-emerald-900 transition-colors w-full sm:w-auto"
                        >
