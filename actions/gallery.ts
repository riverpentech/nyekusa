...
export async function uploadProfilePictureAction(formData: FormData) {
    try {
        ...
        // Generate collision-free filename with prefix "profiles/"
        const originalExtension = file.name.split(".").pop() || "jpg";
        const cleanName = file.name
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, "-")
            .slice(0, 30);
        const uniqueId = Math.random().toString(36).substring(2, 8);
        const fileName = `profiles/avatar_${Date.now()}_${uniqueId}_${cleanName}.${originalExtension}`;
        ...
        console.error("Error in uploadProfilePictureAction:", err);
